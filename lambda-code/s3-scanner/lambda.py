...
def create_dir(input_bucket, input_key, download_path):
    ...
    if not os.path.exists(full_path):
        try:
            ...
            logger.info("Created dir "+full_path)
        except OSError as e:
...
def freshclam_update(input_bucket, input_key, download_path, definitions_path):
    ...
    try:
        ...
        if update_summary.returncode != 0:
            ...
            logger.info("Updated definitions")
            # Iterate directory
            res = []
            for file_path in os.listdir(definitions_path):
                # check if current file_path is a file
                if os.path.isfile(os.path.join(definitions_path, file_path)):
